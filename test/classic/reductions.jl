...
@testset "New compiled pattern matcher" begin
  ...
  @test something(sf) == :(ok())
end

@testset "PatSegment as tail" begin
  r = @rule f(~x, ~~y) => Expr(:call, :ok, (~~y)...)
  sf = r(:(f(1, 2, 3, 4)))
  @test something(sf) == :(ok(2, 3, 4))

  r = @rule x y f(x, 2, y...) => Expr(:call, :ok, y...)
  sf = r(:(f(1, 2, 3, 4)))
  @test something(sf) == :(ok(3, 4))

  sf = r(:(f(1, 2, 3)))
  @test something(sf) == :(ok(3))
...
@testset "Multiple PatSegments" begin
  ...
  sf = r(:(f(1, 2, 3, 4)))
  @test something(sf) == :(ok(1, 2, 3, 4, yeah()))

  r = @rule f(~~x, 3, ~~y) --> ok(~~x, yeah(~~y))
  sf = r(:(f(1, 2, 3, 4, 5)))
  @test something(sf) == :(ok(1, 2, yeah(4, 5)))

  r = @rule f(~~x, 3, ~~y, 5, ~~z) --> ok(~~x, yeah(~~y), ~~z)
  sf = r(:(f(1, 2, 3, 4, 5, 6)))
  @test something(sf) == :(ok(1, 2, yeah(4), 6))

  r = @rule f(~~x, 3, ~~y, 5, ~~z, 7) --> ok(~~x, yeah(~~y), ~~z)
  sf = r(:(f(1, 2, 2, 3, 4, 4, 5, 6, 7, 7)))
  @test something(sf) == :(ok(1, 2, 2, yeah(4, 4), 6, 7))
end
...
function test()
  @test (@rule Qux(1, 2) => "hello")(Qux(1, 2)) |> something == "hello"
  @test (@rule Qux(1, 2) => "hello")(Qux(3, 4)) === nothing
  @test (@rule Qux(1, 2) => "hello")(1) === nothing
  @test (@rule 1 => "hello")(1) |> something == "hello"
...
function test()
  @test (@rule Lux(1, 2) => "hello")(Lux(1, 2)) |> something == "hello"
