...
@testset "Theories" begin
	...
	@test (theory_macro.patternblock == theory)
end
...
@testset "Reduction loop should error. Bound on iterations."
t = @theory begin
    a + b → b + a
end

@test_throws Exception a + b t
...
@test (@reduce $(Airplane()) * b t) == "flies"
@test (@reduce $(Car()) * b t) == "doesnt_fly"
...
@test (@reduce (ε ⋅ a ⋅ ε ⋅ b ⋅ c ⋅ (ε ⋅ ε ⋅ d) ⋅ e) free_monoid) == :abcde
