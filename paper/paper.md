...
![These four e-graphs represent the process of equality saturation, adding many equivalent ways to write $a \times (2 * 3) / 6$ after each iteration. Dashed boxes represent equivalence classes (e-classes) and the other boxes represent e-nodes. The arrow from an e-node to an e-class means that the pointed e-class is a children of the e-node where the arrow starts. \label{fig:egraph}](egraphs.png)
