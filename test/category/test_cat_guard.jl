...
push!(
  ...
  MultiPatRewriteRule(
    @pat((f ⊗ₘ h) ⋅ σ(a, b)),
    @pat(σ(dom(type(f)), dom(type(h))) ⋅ (h ⊗ₘ f)),
    [PatEquiv(@pat(cod(type(f))), @pat(a)), PatEquiv(@pat(cod(type(h))), @pat(b))],
  ),
  MultiPatRewriteRule(
    @pat(σ(c, d) ⋅ (h ⊗ₘ f)),
    @pat((f ⊗ₘ h) ⋅ σ(cod(type(f)), cod(type(h)))),
...
push!(
  cart_rules,
  MultiPatRewriteRule(@pat(Δ(a) ⋅ (f ⊗ₘ k)), @pat(pair(f, k)), [PatEquiv(@pat(dom(type(f))), @pat(dom(type(k))))]),
)
...
macro calc(e...)
  ...
  for (a, b) in zip(e.args[1:(end - 1)], e.args[2:end])
    # println(a, " =? ", b)
    params = SaturationParams(timeout = 12, eclasslimit = 12000, scheduler = BackoffScheduler)
    g = EGraph()
