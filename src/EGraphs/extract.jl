...
function find_costs!(extractor::Extractor{CF,CT}) where {CF,CT}
  ...
  while did_something
    ...
    for (id, eclass) in extractor.g.classes
      ...
      if min_cost != typemax(CT) && (!haskey(extractor.costs, id.val) || (min_cost < extractor.costs[id.val][1]))
        extractor.costs[id.val] = (min_cost, min_cost_node_idx)
        did_something = true
      end
    end
  end
  ...
  for (id, _) in extractor.g.classes
    if !haskey(extractor.costs, id.val)
