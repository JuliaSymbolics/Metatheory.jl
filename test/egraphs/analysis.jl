...
function EGraphs.make(g::EGraph{ExpressionType,NumberFoldAnalysis}, n::ENode) where {ExpressionType}
  istree(n) || return head(n) isa Number ? NumberFoldAnalysis(head(n)) : nothing
  if is_function_call(n) && arity(n) == 2
    op = head(n)
    args = children(n)
